...
def modelo_walfisch_rooftop(hb, h, hm, f, d, b=45):
    # b é ditancia entre construções m
    # h é a altura dos telaho m
    # hm é a altura da antena receptora m
    # hb altura do transmissor
    # d = distancia de vidad direta
    A = 5 * np.log10(((b / 2) ** 2) + ((h - hm) ** 2)) - 9 * np.log10(b) + 20 * np.log10(np.arctan((2 * (h - hm) / b)))
    lex = 57.1 + A + np.log10(f) + 18 * np.log10(d) - 18 * np.log10(hb - h) - 18 * np.log10(
        1 - ((d ** 2) / (17 * (hb - h))))
    return lex


def ikegami_model(h, hr, f, w=22, lr=2, th=np.pi / 2):  # f em MHz
    # h, hr em m e f em MHz
    # th é o angulo entre linha de visada e a rua dado em radianos pois é argumento de np.sin()
    # w é a largura da rua
    l = - 5.8 - 10 * np.log10(1 + (3 / (lr ** 2))) - 10 * np.log10(w) + 20 * np.log10(h - hr) + 10 * np.log10(np.sin(th)) + 10 * np.log10(f)
    return l


def cost_walfisch_ikegami(hb, hm, f, d, h, b=45, ori=90, w=None, city="m"):
    # b é ditancia entre construções m
    # h é a altura dos telaho m
    # hm é a altura da antena receptora m de 1 a 3
    # hb altura do transmissor, de 4 a 50
    # d = distancia de vidad direta
    # w = largura da rua b/2
    # lB é perda em espaço livre
    # city g para grande e m para média
    if w is None:
        w = b / 2
    if (ori >= 0) and (ori < 35):  # graus
        lori = -10 + 0.354 * ori
    elif (ori >= 35) and (ori < 55):
        lori = 2.5 + 0.075 * (ori - 35)
    else:
        lori = 4.0 - 0.114 * (ori - 55)

    if hb > h:
        kd = 18
        ka = 54
        lbsh = -18 * np.log10(1 + (hb - h))
    else:
        if d >= 0.5:  # km
            ka = 54 - 0.8 * (hb - h)
        else:
            ka = 54 - 0.8 * (hb - h) * d / 0.5
        kd = 18 - 15 * (hb - h) / h
        lbsh = 0
    ...
    lrst = -16.9 - 10 * np.log10(w) + 10 * np.log10(f) + 20 * np.log10(h - hm) + lori
...
def longLq_rice_model(f, hg1, hg2, he1, he2, d, yt, qs, dl1, dl2, Dh, visada, h,
                      teta1, teta2, d_urb, hb_urb, urbam='n', polarizacao='v'):  # de 20M a 20 GHz, f em MHz
    # Constantes Gerais #
    ...
    if (urbam == 'u') and (h > 0):
        ub = urbam_factor(d, f)
    elif (urbam == 'w') and (h > 0):
        ub = modelo_walfisch_rooftop(hb_urb, h, hg2, f, d)
    elif (urbam == 'i') and (h > 0):
        ub = ikegami_model(h, hg2, f, )
    elif (urbam == 'wi') and (h > 0):
        if h > (hg2 + 1):
            ub = cost_walfisch_ikegami(hb_urb, hg2, f, d_urb, h)
        else:
            ub = 0
    else:
        ub = 0
