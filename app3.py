...
def R(lat):
    return (((((a**2)*np.cos(lat*np.pi/180))**2)+(((b**2)*np.sin(lat*np.pi/180))**2))/(((a*np.cos(lat*np.pi/180))**2)+((b*np.sin(lat*np.pi/180))**2)))**0.5
...
def perfil(r, src, src_dsm, src_landcover):
    ...
    for i in range(np.shape(r)[0]):
        ...
        dist = distancia*i
        alt_dem = raster[int(pixel_y1)][int(pixel_x1)] #((1 - (pixel_y1 - np.floor(pixel_y1))) * raster[int(np.floor(pixel_y1))][int(np.floor(pixel_x1))] + (
        ...
        alt_dsm = raster_dsm[int(pixel_y1_dsm)][int(pixel_x1_dsm)]#((1 - (pixel_y1_dsm - np.floor(pixel_y1_dsm))) * raster_dsm[int(np.floor(pixel_y1_dsm))][
